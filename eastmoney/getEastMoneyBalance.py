##
# 从东方财富爬取资产负债表。 
# 爬取链接为：http://f10.eastmoney.com/NewFinanceAnalysis/zcfzbAjaxNew?companyType=4&reportDateType=0&reportType=1&dates=1997-12-31%2C1997-06-30%2C1996-12-31%2C1996-06-30%2C1995-12-31&code=SH600176
##
...
def crawlBalance(code, companyType):
    records = []
    for date in DATES:
        link = "http://f10.eastmoney.com/NewFinanceAnalysis/zcfzbAjaxNew?companyType=%d&reportDateType=0&reportType=1&dates=%s&code=%s" % (companyType, date, code)
        session = HTMLSession()
        ...
        FileLogger.info("get balance of code: %s in size: %d" % (code, len(jsonContent["data"])))
        # time.sleep(0.5)
