##
# 从东方财富爬取现金流量表。 
# 爬取链接为：http://f10.eastmoney.com/NewFinanceAnalysis/xjllbAjaxNew?companyType=4&reportDateType=0&reportType=1&dates=2017-06-30%2C2017-03-31%2C2016-12-31%2C2016-09-30%2C2016-06-30&code=SZ002127
##
...
# history Cashflow data till 2021/03/24
def crawlCashflow(code, companyType):
    records = []
    for date in DATES:
        link = "http://f10.eastmoney.com/NewFinanceAnalysis/xjllbAjaxNew?companyType=%d&reportDateType=0&reportType=1&dates=%s&code=%s" % (companyType, date, code)
        session = HTMLSession()
        ...
        FileLogger.info("get Cashflow of code: %s in size: %d" % (code, len(jsonContent["data"])))
        # time.sleep(0.5)
...
if __name__ == "__main__":
    ...
    for list in stockList:
        ...
        FileLogger.info("running on code: %s" % code)
        try:
            crawlCashflow(code, companyType)
            time.sleep(1)
