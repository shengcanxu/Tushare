...
def createColunnIfNotExists(column, type='double'):
    ...
    sqlstr = "select count(*) from information_schema.columns where table_schema='eastmoney' and table_name = 'balance' and column_name = '%s'"
    sqlstr = sqlstr % column
    cursor = ENGINE.execute(sqlstr)
    (result,) = cursor.fetchone()
    if result == 0:
        addColumnStr = "alter table eastmoney.balance add column %s %s DEFAULT NULL;"
